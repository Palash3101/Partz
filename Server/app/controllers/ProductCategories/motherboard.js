...
async function GetMotherboardData(req, res){
  try{
    //Query
    const st = `SELECT
      motherboard.id, \`name\`, price, rating, \`socket\`, chipset, form_factor,P.product_type_name,
    ...
    FROM
      \`PCI-e_lookup\`, motherboard, products, ports_lookup, memory_lookup, storage_lookup, product_type_lookup as P
    WHERE
      ...
      motherboard.id = ports_lookup.motherboard_id AND

...
function GridCard(props) {
  ...
  function DLSS_FSR_Rendering(){
    if (props.data.DLSS_version===null){
      return (
        <div className='text-base'>
        <span className='font-medium'>FSR: </span>{props.data.FSR_version}
        </div>
      )}
    else{
      return(
      <div className='text-base'>
      <span className='font-medium'>DLSS: </span>{props.data.DLSS_version}
      </div>)
    }
    }
      ...
      <div className='col-span-1 flex flex-col justify-center'>
        ...
        <div className='mt-2'>
          <div className='ml-4 grid grid-cols-2 grid-rows-3 gap-y-1 gap-x-2'>
            <div className='text-base'>
              <span className='font-medium'>Core Clock: </span>{props.data.boost_clock}GHz
            </div>  

            <DLSS_FSR_Rendering />

            <div className='text-base'>
              <span className='font-medium'>Cores: </span>{props.data.cuda_cores}
            </div>
            <div className='text-base'>
              <span className='font-medium'>TDP: </span>{props.data.TDP} W  
            </div>
            <div className='text-base col-span-2 truncate'>
              <span className='font-medium'>VRAM: </span>{props.data.vram} GB {props.data.vram_version}
            </div>
          </div>
        </div>
      </div>
